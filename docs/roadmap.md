...
- [x] Objective-card standard: visible objective, live success condition, enumerated failure modes, telegraphed warnings (issue #13 milestone A; sim-side contract read model in `src/sim/contract.ts`, latched loss reason codes, HUD card and captioned comms warnings driven only by the shared queries, abort/recall command books an exfil-with-objective-incomplete loss that keeps surviving assets)
...
- [x] Six new contract types: Sabotage, Convoy Interception, Escort, Asset Recovery, Blackout, Counter-Broadcast (issue #13 milestone C; sabotage plants long-fuse charges inside patrol windows with guard discovery and defusal, convoy interception drives an armored `VEH_CONVOY` toward a district exit with a visible exit countdown and a carried cargo crate, escort moves a pulse-immune fragile asset to a delivery zone through seeded ambushes, asset recovery holds a captured agent (loadout snapshot from `MetaState.captured`) behind a breachable cell door under an execution countdown, blackout relays shrink hostile perception as they fall and summon a grid-restoration crew, counter-broadcast towers strip the persuaded swarm and drive the saturation meter; failure modes, warnings, and progress all live in the contract read model with palette entries and minimap marks for every new marker)
- [x] Distinct interaction verbs per objective (issue #13 milestone C; `breach`, `hack`, `carry`, and `drive` join `persuade` and `place` in the command union with replay self-equality coverage; breach is loud timed demolition, hack-and-hold is the silent relay kill, carry slows the porter and drops on death, drive is the explicit vehicle waypoint order)
...
- [x] Doctrine matrix enforced per contract type (issue #13 milestones B and C; the briefing doctrine read and revealed counters ship on the equip screen, and `tests/doctrineMatrix.test.ts` runs headless bot probes per doctrine per contract type asserting at least three of four doctrines win each of the 13 types)

...
Snapshot as of 2026-07-05, updated during the Phase D implementation pass (partial: see the Phase D section). Verified against the working tree and a passing `npm test` (determinism replays for all seven mission types plus pinned golden hash, world-generation, economy, and map/vehicle/environment unit tests). Sources: `docs/design.md` (GDD), the MVP build plan, and the M0 slice plan. Performance measurements live in `docs/perf.md`.

## 1. Where the project stands

The MVP vertical slice defined in GDD section 12 is functionally complete, Phase B (content depth) and Phase C (world and campaign) are fully in, and Phase D (city simulation) is partially in: mission time-of-day and weather with real perception effects, autonomous vehicles with hijacking and chain-reacting explosions, and a destructible storefront mid-layer, all rendered with per-time-of-day lighting. The architecture rule that could not be retrofitted (deterministic fixed-point command-driven sim) is in place and test-enforced. The Phase A remainder is the manual perf run on a Windows iGPU laptop. The Phase D remainder (GPU-instanced skeletal crowds with LOD, and the flow-field density decision) is not yet done.
...
### Phase D — City simulation upgrades [PARTIAL]
...
- [TODO] GPU-instanced skeletal animation for crowds with LOD behavior tiers. Not started; the crowd is still the single non-animated box `InstancedMesh` capped at 400. A procedural-humanoid VAT approach was scoped (build a rigged low-poly humanoid in code, bake per-frame vertex positions into a DataTexture sampled in a TSL `positionNode`, near/far LOD partition) but is not implemented
- [TODO] Flow fields for flashmob-scale pathfinding if A* breaks down at density (see M2 note). Deferred pending the density probe; not started. Warning: `src/sim/path.ts` uses module-level reusable buffers and is deliberately non-reentrant; any pathfinding change must preserve determinism and the single-call-per-tick assumption
- Determinism: `GOLDEN_FINAL_HASH` re-pinned three times over this pass (once per sim commit: env fields, vehicle/agent folds plus vehicle-spawn rand, breach fold), now `0x913f91e4`; self-equality replays cover vehicle commands (hijack/drive/exit, chain staggering, run-over) and night-rain stealth
- Deviations: vehicles provide cover by blocking projectiles physically, not by blocking line of sight (so targeting can "see" through a car, the bullet then hits it); day is the sim-neutral baseline (night/rain are strictly player-favorable modifiers); tram lines are fixed, not procedurally placed. Balance/density probes were not run this pass
- Verification gap: the sim-layer work (vehicles, breaches, environment) is covered by passing determinism and unit tests, but the render and app commits (vehicle/breach/rubble meshes, per-time-of-day lighting, hijack UX, brief forecast) are verified only by `tsc --noEmit` and the production build. The game was not run in a browser this pass, so nothing visual or interaction-level (mesh appearance, LOD boundaries, cloak flicker, the J/RMB hijack flow, siege/defense UI under the new lighting) has been exercised on WebGPU or the WebGL fallback

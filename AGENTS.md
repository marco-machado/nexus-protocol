...
- `map.ts`: procedural building/street layout (seeded, tunable via the `MapParams` region grammar: block scale, street width, alley density, height distribution, landmark kind), exactly one region-specific landmark anchor per district (obstacle plus `streetBlocked` patch), walkable/edge cell lists, and Bresenham-based line-of-sight. `tests/map.test.ts` pins the default obstacle-grid hash so generation can't silently change.
- `queries.ts`: the shared read-only order-validity query surface (ADR-0002): denial reasons, engagement/persuade/hijack/place/route queries, pure over `SimState` (no mutation, no rand); `tick.ts` accepts commands through the same helpers, so the app-layer intent cursor can never disagree with the sim.
- `district.ts`: the district tactical read: a pure function from the generated map to measured alley/plaza/street facts and per-player-doctrine favor/resist ratings, consumed by briefing intel.
...
- `meta.ts` — `MetaState`: persistent progression (credits, territories, agent roster, R&D board state), serialized to `localStorage` (`saveMeta`/`loadMeta`). `buildSpec` turns a persisted `MetaAgent` into a sim-ready `AgentSpec` (pure loadout/gear/augs; no per-agent quirks). `agentAppearance` builds the shared appearance manifest. `tests/meta.test.ts` covers income cycles, territory acts, mission conditions, uniform-asset behavior, and the R&D board.
- `research.ts` — the R&D project board: the `PROJECTS` data table (each entry a named deliverable with cost, real-time duration, prerequisite edges, and an output-law annotation), lab-slot allocation (`startProject`/`tickProjects` on the injected clock, capped by the same 24h offline rule as income), breakthrough offers seeded from mission facts at debrief time, per-territory `INFRA` purchases with named capabilities, and the in-place migration of pre-board point-pool saves (banked points liquidate at 10cr per point).
...
- `ui/` — one React component per meta screen (menu, world map, equip, settings panel, debrief, victory, region vignette, Corporate Archive) plus `root.tsx`, which mounts the React root on the `#screen` overlay and subscribes to the `ScreenStore`. This is the convention to follow when adding screens; the in-mission HUD stays imperative DOM.
...
- `tutorial.ts` — contextual, predicate-gated `TutorialHint`s (`hintsFor(meta, territory, missionType)`); at mission time these are absorbed into the narrative channel as low-priority entries so the ticker stays one interleaved voice.
- `narrative/` — the narrative layer (GDD drafts 34-38), app-layer only with zero sim impact. `engine.ts` is the one selection mechanism for all five surfaces: entries carry predicate, speaker, priority, scope, and one-shot flag; selection is deterministic (priority then stable order over a fact snapshot plus persisted fired-history, no randomness) and `createNarrativeChannel` rate-limits delivery to one line per beat window with priority preemption. `facts.ts` derives mission facts read-only from `SimState` (audio-engine pattern) and campaign facts from `MetaState`. `history.ts` is the persisted campaign history on the meta save (fired one-shots, vignettes, arc stages, Archive unlocks/unread, memory counters) with in-place upgrade for pre-narrative saves; NG+ resets barks/vignettes/arc stages and preserves the Archive. Content lives in data tables separate from the engine — `cast.ts` (fixed handler-side cast with voice notes), `barks.ts` (mission bark database incl. doctrine-tagged rival lines), `rivalArcs.ts` (arc-stage derivation and debrief campaign lines with memory), `vignettes.ts` (region-opening transcripts), `archive.ts` (Corporate Archive docs with unlock predicates) — and every line is held to Principle III tone; `tests/narrativeContent.test.ts` audits structure and register.
...
- `camera.ts`: isometric camera rig at fixed handler pitch with continuous eased rotation (shortest wraparound arc, see `updateRig`/`rigYawDelta`/`rotateBy`), eased clamped zoom (`viewHeight`/`viewTarget`), and inertial pan velocities; the default frame is orthographic, with a perspective tilt-shift frame prototype behind `?tiltshift` (ADR-0003, open). `tests/camera.test.ts` covers easing, wraparound, zoom clamping, inertia, and the fixed-pitch guarantee.
